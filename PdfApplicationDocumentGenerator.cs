...
namespace Nml.Improve.Me
{
	public class PdfApplicationDocumentGenerator : IApplicationDocumentGenerator
	{
		...
		{
			if (dataContext.Equals(null))//if dataContext is null, changed from '!=' 
				throw new ArgumentNullException(nameof(dataContext));

			DataContext = dataContext;

			//_templatePathProvider = templatePathProvider == null ?? throw new ArgumentNullException("templatePathProvider");
			_templatePathProvider = (templatePathProvider == null) ? throw new ArgumentNullException("templatePathProvider") : templatePathProvider;
			//View_Generator = viewGenerator;
			View_Generator = (viewGenerator == null) ? throw new ArgumentNullException("viewGenerator") : viewGenerator;
			//_configuration = configuration;
			_configuration = (configuration == null) ? throw new ArgumentNullException("configuration") : configuration;
			//_logger = logger ?? throw new ArgumentNullException(nameof(logger));/
			_logger = (logger == null) ? throw new ArgumentNullException("logger") : logger;
			//_pdfGenerator = pdfGenerator;
			_pdfGenerator = (pdfGenerator == null) ? throw new ArgumentNullException("pdfGenerator") : pdfGenerator;
		}
		...
		public byte[] Generate(Guid applicationId, string baseUri)
		{
			...
			if (application != null)
			{
				...
				string view = ""; //assigning null string value
				string path = ""; //save memory to create path once instead of in each application state

				if (application.State == ApplicationState.Pending)
				{
					path = _templatePathProvider.Get("PendingApplication"); //removed creating path variable and assigning
					...
					view = View_Generator.GenerateFromPath(string.Format("*{0~1	09}{1}*", baseUri, path), vm);
				...
				{
					path = _templatePathProvider.Get("ActivatedApplication"); //removed creating path variable and assigning
				...
				else if (application.State == ApplicationState.InReview)
				{
					...
					var inReviewMessage = "Your application has been placed in review" +
										application.CurrentReview.Reason switch
										{
											...
											_ =>
												" because of suspicious account behaviour. Please conta*-ct support ASAP."
